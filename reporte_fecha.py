...
def procesar_datos_racks(buffer_data, recetas, fecha_inicio, fecha_fin):
    """Procesa los datos del buffer para generar el reporte, filtrando por fechas."""
    racks_data = []
    niveles_data = []
    ...
    for buffer_item in buffer_data:
        ...
        if not (fecha_inicio_dt <= fecha_item_inicio_dt <= fecha_fin_dt):
            continue
        ...
        tiempo_neto = calcular_tiempo_diferencia(fecha_item_inicio, fecha_item_fin)
        
        fecha_inicio_str = fecha_item_inicio if isinstance(fecha_item_inicio, str) else ""
        fecha_fin_str = fecha_item_fin if isinstance(fecha_item_fin, str) else ""
...
def export_racks_to_excel_por_fecha(file_path, tabla_buffer, tabla_receta, region, fecha_inicio, fecha_fin):
    """
    Genera un Excel con el reporte de racks filtrados por fecha.
    Args:
    ...
    """
    print("Exportando datos de racks a Excel por fecha...")
    ...
    try:
        racks_data, niveles_data = procesar_datos_racks(buffer_data, recetas, fecha_inicio, fecha_fin)
        
        if not racks_data:
            print(f"No se encontraron registros entre {fecha_inicio} y {fecha_fin}")
            return False
...
if __name__ == "__main__":
    ...
    # Generar archivo Excel (ejemplo con fechas de hoy)
    from datetime import datetime, timedelta
    hoy = datetime.now().strftime("%d-%m-%Y")
    ayer = (datetime.now() - timedelta(days=1)).strftime("%d-%m-%Y")
    
    output_path = "reporte_racks_fecha.xlsx"
    export_racks_to_excel_por_fecha(output_path, TABLA_BUFFER, TABLA_RECETA, REGION, ayer, hoy)
